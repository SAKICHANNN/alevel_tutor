...
| **W3.10** | **两阶段检索 Reranker** | ✅ 代码已实现（`agent/retrieval/reranker.py`），搜索接口已集成 `use_rerank=True`。模型首次下载被 GFW 阻塞——需在有网络的机器下载 `Qwen/Qwen3-Reranker-0.6B`（~1.2GB），之后自动从缓存加载。详细设计见 §4.2B |

**W3 退出标准**：4 科各 20 题 OCR，按题面截图/拍照/手写答案分层统计；题面截图与清晰拍照低置信率 < 20%，手写答案允许更高低置信率但必须可解释降级。Reranker 代码就绪，模型下载后即可启用。

**W3 退出标准**：4 科各 20 题 OCR，按题面截图/拍照/手写答案分层统计；题面截图与清晰拍照低置信率 < 20%，手写答案允许更高低置信率但必须可解释降级。引用 chip 出现在每个正式答案末尾。

### Week 4：批改校准 + 数据库 + Reranker 上线

**目标**：批改和真人评分对标，错题日志入库，Reranker 模型下载并接入生产检索。
...
| **W4.8** | **Reranker 模型下载 + 接入生产检索** | `huggingface_hub.snapshot_download('Qwen/Qwen3-Reranker-0.6B')` 下载成功；Agent 默认 `use_rerank=True`；RAG 精度对比测试（rerank vs no-rerank） |
